...
Interpret PATTERN as a basic regular expression.
 ...
 -s, --suffiv=SUFFIX        find files which end with SUFFIX
 -i, --ignore-case          igonore case distincions
...
parameters=$(getopt -n "$SCRIPT_NAME" -o d:s:in -l directory: -l suffix: -l ignore-case -l line-number -l help -l version -- "$@")
...
find -- "$directory" -type f -name "$find_name" -print0 | xargs -0 grep "$grep_option" -e "$pattern" -- /dev/null
